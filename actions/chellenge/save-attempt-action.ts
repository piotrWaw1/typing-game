...
export default async function saveAttemptEasy(correct: number, incorrect: number) {
  ...
  const { error } = await supabase.from('easy_attempt').insert({ accuracy: value, user_name: user.email });
